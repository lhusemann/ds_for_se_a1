...
# idk we run out of memory otherwise so we use it...
# ==========================================
...
def get_descriptive_summary_zero_shot_prompt(user_content):
    return [
        {"role": "system", "content": "You are a helpful data scientist for software engineering. Your job is to create a descriptive summary and a title explaining a software module’s overall behaviour, architecture, and how the components interact within the cluster. The summary should include Components and Interactions (How the distinct parts of the cluster work together), Quality Attributes (The non-functional requirements achieved by this architecture like e.g. scalability, security, maintainability) and Technology Used (Any frameworks, languages, or tools identified in the summaries). Don't forget to give the whole descriptive summary a fitting title. Please keep this description under 150 Words!"},
...
def do_llm_call(prompt) -> str:
    ...
    inputs = tokenizer.apply_chat_template(
        ...
        return_tensors="pt",
        truncation=True,
        max_length=7680 # some files were too big, so I accept some loss to even run the task
    ).to(model.device)
...
BASE_OUTPUT_DIR = Path("/scratch/hpc-prf-dssecs/group-9/extra_hierarchical_summarization")
